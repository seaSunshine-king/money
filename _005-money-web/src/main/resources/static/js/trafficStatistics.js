...
$(function(){
	...
	$(".logged").hover(function(){
		$.get(rootPath+"/loan/page/balance",function (message){
			if(message.code==0){
				$("#frame_top").html(message.message);
			}
			if(message.code==1){
				var money = message.message;
				/*超过一万，不直接显示余额*/
				if(money>=10000){
					$("#frame_top").prop("value",message.message);
					$("#frame_top").html("*****");
				}else {
					$("#frame_top").html(message.message);
				}
...
$(function (){
	/*鼠标悬停在余额上显示余额*/
	$("#frame_top").hover(function (){
		var money = $("#frame_top").val();
		$("#frame_top").html(money);
	});
	...
	/*$("showBalance").click(function (){
		alert(111)
		var money = $("#showBalance").val();
		$("#showBalance").html(money);
	});*/
});